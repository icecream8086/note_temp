...
def get_responses(system_info, char_arrays, max_threads=3):
    """
    并发处理每个字符数组，并将结果替换原来的内容。

    :param system_info: 系统提示信息
    :param char_arrays: 字符数组列表
    :param max_threads: 最大线程数，默认为3
    """
...
def process_all_srt_files(srt_folder, output_folder):
    """
    处理指定文件夹下的所有SRT文件，并将结果保存到输出文件夹。

    :param srt_folder: 包含SRT文件的文件夹路径
    :param output_folder: 输出Markdown文件的文件夹路径
    """
    # 确保输出文件夹存在
    ...
    for filename in os.listdir(srt_folder):
        if filename.endswith(".srt"):
            ...
            print(f"正在处理文件: {srt_file_path}")

            # 处理SRT文件
            char_arrays = process_srt_file(srt_file_path)
...
# 设置SRT文件夹和输出文件夹路径
srt_folder = "srt"  # SRT文件所在的文件夹
output_folder = "mdoc"  # 输出Markdown文件的文件夹

# 处理所有SRT文件
process_all_srt_files(srt_folder, output_folder)